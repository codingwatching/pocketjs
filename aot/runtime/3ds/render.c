// aot/runtime/3ds/render.c — the software renderer.
//
// Decodes the GBA-format PJGB assets (4bpp tiles, BGR555 palettes) straight
// out of the cart blob every frame:
//   top    (200x120) — camera window over the map + 16x16 sprites, exactly
//                      the composition BG0+OBJ produce on the GBA
//   bottom (320x240) — textbox / choice menu, drawn from textbox.c state
//                      with cjk16 halfcell glyphs pulled from the GLYPHS
//                      chunk (no slot streaming: we have no VRAM to ration)
#include "runtime.h"
