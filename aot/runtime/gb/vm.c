// aot/runtime/gb/vm.c — the event-script stack machine (Game Boy port).
//
// Bytecode is copied from banked ROM into a WRAM buffer at vm_start, so the
// interpreter never juggles banks mid-run. Semantics mirror the GBA
// runtime/gba/script_vm.c op for op (the cross-target contract is the E2E
// suite driving identical scenarios on both).
#include <string.h>
#include "gbrt.h"

static u8 rd_u8(void) { return pj_script_ram[g.vm.ip++]; }
static u16 rd_u16(void) {
  u16 v = pj_script_ram[g.vm.ip] | ((u16)pj_script_ram[g.vm.ip + 1] << 8);
  g.vm.ip += 2;
  return v;
}
...
static void face_player(s8 slot) {
  ...
  ax = (s16)pj_actors[(u8)slot].x;
  ay = (s16)pj_actors[(u8)slot].y;
  px = g.px >> 3;
  py = g.py >> 3;
  dx = px - ax;
  dy = py - ay;
...
void vm_start(u8 script_id, s8 actor_slot) {
  u16 off, end;
  SWITCH_ROM(BANK(pj_scripts));
  off = pj_script_offs[script_id];
  end = pj_script_offs[script_id + 1];
  memcpy(pj_script_ram, pj_scripts + off, end - off);
  g.vm.ip = 0;
  g.vm.sp = 0;
  g.vm.active = 1;
