#!/usr/bin/env bun
// aot/test/harness/ds_runner.ts — headless DS runner speaking the same
// scenario JSON protocol as mgba_runner / nes_runner / 3ds_runner:
//
//   bun ds_runner.ts <game.nds> <scenario.json>
//
// There is no scriptable DS emulator wired in, so this drives the runtime's
// HOST build: targets/nds.ts compiles the identical core (game logic, VM,
// software renderer, debug block) into <game>.host.dylib next to the .nds,
// and this runner ticks it over Bun FFI. Only render_ds.c + nds_main.c (the
// libnds hardware/present shell) are outside the loop.
//
// Debug reads translate scenario bus addresses as (addr - debugAddr) into the
// core's exported block. Screenshots stack both 256x192 screens into one PPM.
...
if (!romPath || !scenarioPath) {
  console.log(JSON.stringify({ ok: false, error: "usage: ds_runner <game.nds> <scenario.json>" }));
  process.exit(1);
}
...
// Core key mask (GBA/DS KEYINPUT bit layout).
...
const spec = TARGETS["nds"];
const TOP_W = spec.screenW; // 256
const TOP_H = spec.screenH; // 192
const BOT_W = 256;
const BOT_H = 192;

const dylibPath = romPath.replace(/\.nds$/, "") + ".host.dylib";
if (!(await Bun.file(dylibPath).exists())) {
  console.log(JSON.stringify({ ok: false, error: `host dylib not found: ${dylibPath} (build with --target nds)` }));
  process.exit(1);
}
...
async function screenshot(path: string): Promise<void> {
  // Top world viewport is 128x96, shown at 2x on the real top panel — mirror
  // that: 256x192 (top, upscaled) over 256x192 (bottom, 1:1).
  const W = TOP_W * 2; // 256
  const H = TOP_H * 2 + BOT_H; // 192 + 192
  const head = `P6\n${W} ${H}\n255\n`;
  ...
  for (let y = 0; y < BOT_H; y++) for (let x = 0; x < BOT_W; x++) put(x, TOP_H * 2 + y, rgb(bottomFb[y * BOT_W + x]));
