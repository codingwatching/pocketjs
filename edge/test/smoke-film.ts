// edge/test/smoke-film.ts — 2-scene pipeline smoke test (placeholder art).
...
const arcade = defineScene({
  ...
  play: cue(function* () {
    ...
    yield fadeOut(15);
    yield gotoScene("alley");
  }),
});

// action scene: run-and-gun with a gate, thug melee, gunner bursts, a drone,
// the sandevistan slow-mo, and a PCM music track streaming underneath
const alley = defineScene({
  id: "alley",
  sky: gradient("#0a0a20", "#242455", "#583a68"),
  far: image("art/hills.png", { scroll: 0.35 }),
  main: image("art/street.png", { wide: true }),
  backdrop: "#0a0a20",
  actors: {
    runner: sprite("art/runner.png", { w: 32, h: 32, frames: 8 }),
    thug: sprite("art/thug.png", { w: 32, h: 32, frames: 4 }),
    dronex: sprite("art/dronex.png", { w: 16, h: 16, frames: 4 }),
  },
  action: {
    player: { actor: "runner", hp: 6, sande: 24 },
    ground: 128,
    length: 800,
    gates: [{ x: 320, wave: 1 }],
    spawns: [
      { type: "thug", actor: "thug", x: 280, hp: 2, wave: 1 },
      { type: "gunner", actor: "thug", x: 440, hp: 2 },
      { type: "drone", actor: "dronex", x: 580, hp: 2 },
    ],
  },
  play: cue(function* () {
    yield music("smoke");
    yield fadeIn(15);
    yield caption("chip", "ACTION TEST");
    yield meterShow(0, "hp", 16, 4, 6);
    yield meterShow(1, "sande", 16, 14, 24);
    const r = yield action();
    yield setVar("act_result", r);
    yield music("off");
    yield meterHide(0);
    yield meterHide(1);
    yield captionClear("all");
...
// room first: the engine e2e drives the new world/minigame/action paths with
// the shortest possible boot; street/dream keep covering the cine vocabulary.
// arcade jumps to alley explicitly; alley falls through to street in film order.
export default defineFilm({
  title: "EDGE SMOKE",
  scenes: [room, arcade, alley, street, dream],
  music: { smoke: track("art/smoke-track.raw", { loop: true }) },
});
