// edge/test/gen-placeholder-art.ts — procedural PNGs for the smoke film so the
...
// action runner: 32x32 x 8 frames (idle, run x4, jump, shoot, hurt)
await Bun.write(
  DIR + "runner.png",
  img(256, 32, (x, y) => {
    const f = Math.floor(x / 32);
    const lx = x % 32;
    const lean = f >= 1 && f <= 4 ? 2 : 0; // run frames lean forward
    // head
    if (Math.hypot(lx - 16 - lean, y - 8) < 5) {
      if (f === 7) return [220, 120, 120, 255]; // hurt flash
      return [240, 200, 160, 255];
    }
    // body
    if (y >= 13 && y < 23 && lx >= 12 + lean && lx < 20 + lean)
      return f === 6 ? [230, 210, 90, 255] : [90, 200, 220, 255]; // shoot pose pops
    // gun arm on shoot frame
    if (f === 6 && y >= 15 && y < 18 && lx >= 20 && lx < 28) return [40, 48, 60, 255];
    // legs: 4-phase run cycle, tucked when jumping (f5)
    if (y >= 23 && y < 31) {
      if (f === 5) {
        if (y < 27 && (Math.abs(lx - 13) < 2 || Math.abs(lx - 19) < 2)) return [40, 48, 60, 255];
        return [0, 0, 0, 0];
      }
      const phase = f >= 1 && f <= 4 ? f - 1 : 0;
      const spread = [1, 4, 1, 4][phase] + (f >= 1 && f <= 4 ? 1 : 0);
      const flip = phase >= 2 ? -1 : 1;
      if (Math.abs(lx - (16 - spread * flip)) < 2 || Math.abs(lx - (16 + spread * flip)) < 2)
        return [40, 48, 60, 255];
    }
    return [0, 0, 0, 0];
  }),
...
// action thug: 32x32 x 4 frames (idle, walk0, walk1, attack)
await Bun.write(
  DIR + "thug.png",
  img(128, 32, (x, y) => {
    const f = Math.floor(x / 32);
    const lx = x % 32;
    if (Math.hypot(lx - 16, y - 9) < 5.5) return f === 3 ? [255, 120, 90, 255] : [200, 160, 130, 255];
    if (y >= 14 && y < 24 && lx >= 11 && lx < 21) return [160, 60, 70, 255];
    if (f === 3 && y >= 16 && y < 19 && lx >= 21 && lx < 30) return [40, 48, 60, 255]; // lunge arm
    if (y >= 24 && y < 31) {
      const spread = f === 1 ? 4 : f === 2 ? 1 : 2;
      if (Math.abs(lx - (16 - spread)) < 2.5 || Math.abs(lx - (16 + spread)) < 2.5) return [50, 40, 46, 255];
    }
    return [0, 0, 0, 0];
  }),
);

// action drone: 16x16 x 4 frames
await Bun.write(
  DIR + "dronex.png",
  img(64, 16, (x, y) => {
    const f = Math.floor(x / 16);
    const lx = x % 16;
    if (y >= 6 && y < 11 && lx >= 3 && lx < 13) return [120, 130, 150, 255]; // hull
    if (y === 5 && ((f % 2 === 0 && (lx === 2 || lx === 13)) || (f % 2 === 1 && (lx === 4 || lx === 11))))
      return [220, 230, 240, 255]; // rotor blur alternates
    if (f === 3 && y >= 11 && y < 13 && lx >= 7 && lx < 9) return [255, 90, 70, 255]; // firing eye
    if (y >= 8 && y < 10 && lx >= 6 && lx < 10) return [255, 170, 60, 255]; // sensor band
    return [0, 0, 0, 0];
  }),
);

// smoke music track: 2s of square-wave arpeggio, s8 mono @ 13379 Hz
{
  const RATE = 13379;
  const secs = 2;
  const pcm = new Int8Array(RATE * secs);
  const notes = [220, 277.18, 329.63, 440]; // A minor-ish arp
  for (let i = 0; i < pcm.length; i++) {
    const t = i / RATE;
    const f = notes[Math.floor(t * 8) % notes.length];
    const sq = Math.sign(Math.sin(2 * Math.PI * f * t));
    const env = 1 - ((t * 8) % 1) * 0.6;
    pcm[i] = Math.round(sq * 52 * env);
  }
  await Bun.write(DIR + "smoke-track.raw", new Uint8Array(pcm.buffer));
}

console.log("placeholder art written to edge/test/art/");
