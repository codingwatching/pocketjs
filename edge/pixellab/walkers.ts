// edge/pixellab/walkers.ts — assemble top-down walker sheets from the stills.
//   bun pixellab/walkers.ts [--force]
//
// david, lucy: real 4-frame walk cycles per direction via /animate-with-text
//       (64x64 minimum -> 2x nearest-neighbor round trip) -> wd_<who>.png
//       384x32, rows DOWN,UP,SIDE x 4 frames (walkFpd 4). Frame 0 of each
//       row is the standing still.
// maine, rebecca: 3-frame sheets [south, north, east] (walkFpd 1)
// falco, kiwi, gloria, dorio, pilar: south-only stills tripled (walkFpd 1)

import { apiKey } from "./client.ts";
import { decodePng, encodePng } from "../compiler/png.ts";
import { DAVID, LUCY } from "./generate.ts";
...
// leads: 4-frame cycles, standing still as frame 0 of each row
for (const [who, look] of [
  ["david", DAVID],
  ["lucy", LUCY],
] as const) {
  const out = `${ART}wd_${who}.png`;
  if (!force && (await Bun.file(out).exists())) {
    console.log(`skip wd_${who} (cached)`);
    continue;
  }
  ...
  for (const [suffix, dir] of [
    ...
    const still = await loadFrame(`${ART}walk_${who}_${suffix}.png`);
    process.stdout.write(`  animate ${who} ${dir}... `);
    const frames = await animate(still, look, dir);
  ...
  console.log(`wrote wd_${who}.png (12 frames)`);
}

// crew NPCs with real facing stills
for (const who of ["maine", "rebecca"]) {
  ...
  console.log(`wrote wd_${who}.png (3 frames)`);
}

// south-only NPCs: the still tripled so face() is harmless
for (const who of ["falco", "kiwi", "gloria", "dorio", "pilar"]) {
  const out = `${ART}wd_${who}.png`;
  if (!force && (await Bun.file(out).exists())) {
    console.log(`skip wd_${who} (cached)`);
    continue;
  }
  const s = await loadFrame(`${ART}walk_${who}_s.png`);
  await Bun.write(out, sheet([s, s, s]));
  console.log(`wrote wd_${who}.png (3 frames)`);
}
