// edge/dsl/index.ts — the @pocketjs/edge authoring surface.
//
...
// --- action (side-scrolling run-and-gun) declarations ---------------------------
export type EnemyType = "thug" | "gunner" | "drone" | "turret" | "boss";

export interface ActionSpawnDecl {
  type: EnemyType;
  actor: string; // 4-frame side strip (idle, walk0, walk1, attack); boss may be 64px
  x: number; // world px (activates as the player approaches)
  hp?: number;
  wave?: number; // gate tag (see gates)
}

export interface ActionDecl {
  /** actor with the 8-frame side-view strip: idle, run x4, jump, shoot, hurt.
   * Must be the FIRST actor declared in the scene (it lives in sprite slot 0). */
  player: { actor: string; hp?: number; sande?: number };
  ground: number; // ground surface y, world px
  length?: number; // virtual stage px (default: main image width; map wraps at 512)
  platforms?: [number, number, number][]; // x, y(top surface), w — one-way
  gates?: { x: number; wave: number }[]; // hold the player at x while wave lives
  spawns: ActionSpawnDecl[];
  exit?: "end" | "clear"; // reach length (default) | kill every spawn
  /** boss stages: when the boss' hp falls to this, the stage ends immediately
   * with result 2 (ACT_BOSS_PHASE) and the story takes over. */
  bossPhaseHp?: number;
  /** game vars bound to the HUD/state (share the film-wide var pool) */
  vars?: { hp?: string; sande?: string; kills?: string; deaths?: string };
}

export interface TrackDecl {
  kind: "track";
  raw: string; // s8 mono PCM @ 13379 Hz (ffmpeg -ac 1 -ar 13379 -f s8)
  loop?: boolean;
}
...
export interface SceneDecl {
  ...
  world?: WorldDecl;
  /** presence makes this an ACTION scene: `yield action()` runs the stage. */
  action?: ActionDecl;
  play: CueRef;
}
...
export interface FilmDecl {
  title: string;
  scenes: SceneDecl[];
  /** PCM insert songs by name: `music: { stay: track("music/stay.raw", { loop: true }) }` */
  music?: Record<string, TrackDecl>;
}
...
export function track(raw: string, opts: Omit<TrackDecl, "kind" | "raw"> = {}): TrackDecl {
  return { kind: "track", raw, ...opts };
}
...
export function cue(fn: number | (() => Generator<unknown, unknown, unknown>)): CueRef {
  ...
  throw new Error("cue() bodies are residual-only; compile with edge/compiler");
}
...
// world + encounters + minigames + action + music
...
/** blocks on the scene's action stage; returns 1 (cleared) or 2 (boss phase) */
export const action = residual("action") as () => number;
/** start/stop a PCM insert song declared in the film's `music` table */
export const music = residual("music") as (name: string | "off") => R;
