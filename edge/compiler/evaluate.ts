// edge/compiler/evaluate.ts — execute the declaration zone, capture cue ASTs.
...
export async function evaluateFilm(entryPath: string): Promise<EvalResult> {
  ...
  rewritten = rewritten.replace(/(["'])@pocketjs\/edge\1/g, JSON.stringify(DSL_INDEX));
  ...
  const tmp = entryPath + `.__edge.${process.pid}.${evalCounter++}.mjs`;
