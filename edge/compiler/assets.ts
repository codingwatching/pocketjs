// edge/compiler/assets.ts — turn PNGs into GBA-native data: median-cut
...
import { rgb555, hex555, UI_INK, UI_BOX, UI_ACCENT, UI_SHADOW } from "../spec/edge.ts";
...
export function uiObjTiles(): Uint8Array {
  ...
  // 33: player bullet 8x8 — bright ink bolt with an accent tail
  {
    const px = new Array(64).fill(0);
    for (let x = 2; x < 7; x++) px[3 * 8 + x] = px[4 * 8 + x] = UI_INK;
    px[3 * 8 + 1] = px[4 * 8 + 1] = UI_ACCENT;
    px[3 * 8 + 7] = px[4 * 8 + 7] = UI_ACCENT;
    tiles.push(tile4(px));
  }
  // 34: enemy bullet 8x8 — shadow-rimmed hostile pellet
  {
    const px = new Array(64).fill(0);
    for (let y = 2; y < 6; y++)
      for (let x = 2; x < 6; x++) px[y * 8 + x] = UI_SHADOW;
    px[3 * 8 + 3] = px[3 * 8 + 4] = px[4 * 8 + 3] = px[4 * 8 + 4] = UI_INK;
    tiles.push(tile4(px));
  }
  // 35: impact spark 8x8 — 4-point accent star
  {
    const px = new Array(64).fill(0);
    for (let i = 0; i < 8; i++) px[i * 8 + 3] = px[i * 8 + 4] = px[3 * 8 + i] = px[4 * 8 + i] = UI_ACCENT;
    px[3 * 8 + 3] = px[3 * 8 + 4] = px[4 * 8 + 3] = px[4 * 8 + 4] = UI_INK;
    tiles.push(tile4(px));
  }
  // 36-37: ground shockwave 16x8 — low accent crest
  for (const half of [0, 1]) {
    const px = new Array(64).fill(0);
    for (let x = 0; x < 8; x++) {
      const gx = half * 8 + x;
      const h = 2 + ((gx * 5) % 3);
      for (let y = 8 - h; y < 8; y++) px[y * 8 + x] = y === 8 - h ? UI_INK : UI_ACCENT;
    }
    tiles.push(tile4(px));
  }
...
/** Sandevistan palette: desaturated cyan-teal duotone of a BG palette. The UI
 * bank (15) is kept intact so captions/HUD stay readable while time is slow. */
export function sandePalette(palBg: Uint16Array): Uint16Array {
  ...
  for (let i = 0; i < 256; i++) {
    ...
    const c = palBg[i];
    const r = c & 31, gg = (c >> 5) & 31, b = (c >> 10) & 31;
    const luma = (r * 5 + gg * 9 + b * 2) >> 4;
    const nr = Math.min(31, (luma * 5) >> 4);
    const ng = Math.min(31, ((luma * 13) >> 4) + 3);
    const nb = Math.min(31, ((luma * 12) >> 4) + 4);
    out[i] = nr | (ng << 5) | (nb << 10);
  }
  return out;
}
