// edge/compiler/index.ts — compileFilm(entry): evaluate the declaration zone,
...
import {
  ...
  uiPalette, uiBgTiles, uiObjTiles, sandePalette, type Quantized,
} from "./assets.ts";
import {
  ...
  SCENE_CINE, SCENE_WORLD, SCENE_ACTION, CELL_PX, WORLD_COLS_MAX, WORLD_ROWS_MAX,
  MAX_NPCS, MAX_TRIGS, MAX_CUES, TRIG_EXIT, TRIG_EXAMINE, TRIG_AUTO,
  DIR_DOWN, DIR_UP, DIR_LEFT, DIR_RIGHT,
  EB_THUG, EB_GUNNER, EB_DRONE, EB_TURRET, EB_BOSS,
  MAX_SPAWNS, MAX_PLATS, MAX_GATES, MAX_TRACKS,
  AEXIT_END, AEXIT_CLEAR, PLAYER_FRAMES, ENEMY_FRAMES, N_VARS,
} from "../spec/edge.ts";
import type {
  ActionDecl, ActorDecl, CellRef, CueRef, LayerDecl, RectRef, SceneDecl, TrackDecl, WorldDecl,
} from "../dsl/index.ts";
...
export interface CompiledSpawn {
  x: number;
  proto: number;
  behavior: number;
  hp: number;
...
export interface CompiledStage {
  ...
  killsVar: number;
  deathsVar: number;
  spawns: CompiledSpawn[];
  plats: [number, number, number][];
  gates: [number, number][];
  boss: number; // spawn index, 0xff none
  bossPhaseHp: number;
  palSande: Uint16Array;
}

export interface CompiledTrack {
  name: string;
  path: string; // absolute; emitted via .incbin
  samples: number;
  loop: number;
}
...
export async function compileFilm(entryPath: string): Promise<CompiledFilm> {
  ...
  // PCM insert songs: validated here, embedded by emit via .incbin
  const tracks: CompiledTrack[] = [];
  const trackIndex = new Map<string, number>();
  for (const [name, t] of Object.entries(film.music ?? {})) {
    if ((t as TrackDecl).kind !== "track") throw new Error(`music ${name}: use track("file.raw")`);
    if (tracks.length >= MAX_TRACKS) throw new Error(`too many music tracks (max ${MAX_TRACKS})`);
    const p = resolve(base, t.raw);
    const f = Bun.file(p);
    if (!(await f.exists())) throw new Error(`music track ${name}: ${p} not found`);
    if (f.size < 4) throw new Error(`music track ${name}: empty`);
    trackIndex.set(name, tracks.length);
    tracks.push({ name, path: p, samples: f.size, loop: t.loop ? 1 : 0 });
  }

  const scenes: CompiledScene[] = [];
  for (const decl of film.scenes) {
    scenes.push(await compileScene(decl, base, { texts, vars, flags, sceneIndex, cues, trackIndex }));
  }
  ...
  return {
    ...
    debug: {
      ...
      tracks: Object.fromEntries(trackIndex),
    },
  };
}
...
const EB_NUM: Record<string, number> = {
  thug: EB_THUG, gunner: EB_GUNNER, drone: EB_DRONE, turret: EB_TURRET, boss: EB_BOSS,
};
...
function compileAction(
  ...
  actors: CueCtx["actors"],
  protos: CompiledProto[],
  internVar: (name: string) => number,
  palBg: Uint16Array,
): CompiledStage {
  const err = (msg: string): never => {
    throw new Error(`[${sceneId}] action: ${msg}`);
  };
  const pa = actors.get(ad.player.actor);
  if (!pa) return err(`unknown player actor "${ad.player.actor}"`);
  if (pa.slot !== 0) return err(`player actor "${ad.player.actor}" must be the first declared actor`);
  if (protos[pa.proto].frames < PLAYER_FRAMES)
    return err(`player sheet needs ${PLAYER_FRAMES} frames (idle, run x4, jump, shoot, hurt)`);

  const length = ad.length ?? imgW;
  if (length < 240) return err(`stage length ${length} < 240`);
  if (ad.spawns.length > MAX_SPAWNS) return err(`too many spawns (max ${MAX_SPAWNS})`);
  if ((ad.platforms ?? []).length > MAX_PLATS) return err(`too many platforms (max ${MAX_PLATS})`);
  if ((ad.gates ?? []).length > MAX_GATES) return err(`too many gates (max ${MAX_GATES})`);

  const order = ad.spawns.map((s, i) => i).sort((a, b) => ad.spawns[a].x - ad.spawns[b].x);
  let boss = 0xff;
  const spawns: CompiledSpawn[] = order.map((oi, i) => {
    const s = ad.spawns[oi];
    const a = actors.get(s.actor);
    if (!a) return err(`spawn ${oi}: unknown actor "${s.actor}"`);
    if (protos[a.proto].frames < ENEMY_FRAMES)
      return err(`spawn actor "${s.actor}" needs ${ENEMY_FRAMES} frames (idle, walk x2, attack)`);
    const behavior = EB_NUM[s.type];
    if (behavior === undefined) return err(`spawn ${oi}: unknown type "${s.type}"`);
    if (s.type === "boss") {
      if (boss !== 0xff) return err("only one boss per stage");
      boss = i;
    }
    if (s.x < 0 || s.x >= length) return err(`spawn ${oi} x ${s.x} outside stage [0, ${length})`);
    return { x: s.x, proto: a.proto, behavior, hp: s.hp ?? (s.type === "boss" ? 40 : 2), wave: s.wave ?? 0 };
  });

  const gates: [number, number][] = (ad.gates ?? [])
    .map((gt): [number, number] => [gt.x, gt.wave])
    .sort((a, b) => a[0] - b[0]);
  if (ad.bossPhaseHp !== undefined && boss === 0xff) return err("bossPhaseHp without a boss spawn");

  return {
    playerProto: pa.proto,
    hpMax: ad.player.hp ?? 6,
    sandeMax: ad.player.sande ?? 0,
    exitKind: ad.exit === "clear" ? AEXIT_CLEAR : AEXIT_END,
    groundY: ad.ground,
    length,
    hpVar: internVar((ad.vars?.hp ?? "hp")),
    sandeVar: internVar((ad.vars?.sande ?? "sande")),
    killsVar: internVar((ad.vars?.kills ?? "kills")),
    deathsVar: internVar((ad.vars?.deaths ?? "deaths")),
    spawns,
    plats: ad.platforms ?? [],
    gates,
    boss,
    bossPhaseHp: ad.bossPhaseHp ?? 0,
    palSande: sandePalette(palBg),
  };
}
...
async function compileScene(decl: SceneDecl, base: string, env: SceneEnv): Promise<CompiledScene> {
  ...
  if (decl.action) {
    if (decl.world) throw new Error(`[${decl.id}] a scene cannot be both world and action`);
    const internVar = (name: string): number => {
      let id = env.vars.get(name);
      if (id === undefined) {
        id = env.vars.size;
        if (id >= N_VARS) throw new Error(`too many vars (max ${N_VARS}): ${name}`);
        env.vars.set(name, id);
      }
      return id;
    };
    stage = compileAction(decl.id, decl.action, imgW, actors, protos, internVar, palBg);
  }
  ...
  let camMax = decl.camera?.max ?? Math.max(0, imgW - 240);
  if (stage) camMax = Math.max(camMax, stage.length - 240); // virtual stages outrun their art
  ...
  return {
    ...
    mapSky,
    mapSz,
    kind: isWorld ? SCENE_WORLD : decl.action ? SCENE_ACTION : SCENE_CINE,
    world,
    stage,
    cueOffs,
    farFacQ8,
