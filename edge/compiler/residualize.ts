// edge/compiler/residualize.ts — lower cue generator ASTs to cue-VM bytecode.
//
...
export interface CueCtx {
  ...
  cueName: string;
  /** film music tracks: name -> track id */
  ...
  const emitOp = (call: { name: string; args: readonly ts.Expression[] }, n: ts.Node): void => {
    ...
    switch (call.name) {
      ...
      case "music": {
        const name = str(a[0]);
        if (name === "off") {
          w.u8(OP.MUSIC).u8(0xff);
          return;
        }
        const id = ctx.trackIndex.get(name);
        if (id === undefined) return err(n, `unknown music track "${name}" (declare it in the film's music table)`);
        w.u8(OP.MUSIC).u8(id);
        return;
      }
      case "meterShow":
