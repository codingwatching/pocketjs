/* edge/runtime/world.c — top-down grid world: Pokémon-style cell stepping,
...
static u8 npc_at(u8 cx, u8 cy, const EdgeNpc **out) {
  const EdgeWorld *w = wld();
  int i;
...
static const EdgeTrig *trig_at(u8 cx, u8 cy, u8 kind) {
  const EdgeWorld *w = wld();
  int i;
  for (i = 0; i < w->n_trigs; i++) {
    const EdgeTrig *t = &w->trigs[i];
...
void walk_start(u8 slot, u8 cx, u8 cy) {
  const EdgeProto *p = &g.sc->protos[g.spr[slot].proto];
  g.wk_active = 1;
  g.wk_slot = slot;
