/* edge/runtime/debug.c — fixed EWRAM debug block; the E2E contract. */
#include "edge.h"
...
void debug_flush(void) {
  ...
  D16(DBGO_ACT_X) = (u16)act.x;
  D8(DBGO_ACT_ENEMIES) = act.active ? act.alive : 0;
  D8(DBGO_ACT_BOSS_HP) = act.boss_hp;
  D8(DBGO_ACT_SANDE) = act.sande_on;
  D8(DBGO_MUSIC) = music_playing();
}
