...
//! pocketjs-framework PSP host: boots QuickJS on a 2 MB worker thread, evaluates the
...
//! gfx/gfx3d/bridge registrations replaced by the pocketjs-framework stack:
...
unsafe fn trace_write(bytes: &[u8]) {
    let fd = sys::sceIoOpen(
        b"host0:/pocketjs-framework-trace.txt\0".as_ptr(),
...
unsafe fn trace(msg: &str) {
    if trace_enabled() {
        trace_write(b"[pocketjs-framework trace] ");
        trace_write(msg.as_bytes());
        trace_write(b"\n");
        psp::dprintln!("[pocketjs-framework trace] {}", msg);
    }
}
...
unsafe fn log_exception(ctx: *mut JSContext) {
    ...
    if !s.is_null() {
        if let Ok(msg) = core::str::from_utf8(core::slice::from_raw_parts(s as *const u8, len)) {
            trace_pair(b"[pocketjs-framework js error] ", msg);
            psp::dprintln!("[pocketjs-framework js error] {}", msg);
        }
...
// POCKETJS_CAP_N env, baked like the input script): pocketjs-framework's frame 0 lands after
...
unsafe fn halt(msg: &str) -> ! {
    trace_pair(b"[pocketjs-framework halt] ", msg);
    psp::dprintln!("[pocketjs-framework halt] {}", msg);
