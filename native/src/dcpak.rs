    ...
    for i in 0..count {
        ...
        if key == "ui:styles" {
            if !ui.load_styles(blob) {
                psp::dprintln!("[pocketjs-framework dcpak] bad styles.bin");
            }
        } else if key.starts_with("ui:font.") {
            if !ui.load_font_atlas(blob) {
                psp::dprintln!("[pocketjs-framework dcpak] bad font atlas {}", key);
            }
            ...
            if handle >= 0 {
                ...
                psp::dprintln!("[pocketjs-framework dcpak] bad image {} ({}x{} psm {})", key, w, h, psm);
            }
        }
