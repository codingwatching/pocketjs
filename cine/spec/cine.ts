// cine/spec/cine.ts — single source of truth for the @pocketjs/cine binary
// contract: cue bytecode, tween targets, VRAM/palette plan, debug block.
//
// @pocketjs/cine is an INDEPENDENT sibling of @pocketjs/aot: a GBA-only
// cinematic-montage DSL. Scenes are declarative (4-layer parallax, Mode 0);
// each scene's `play` generator is residualized into bytecode for a tiny
// suspendable cue VM. The point is to flex GBA 2D: alpha blending, mosaic,
// affine OBJs, HBlank raster FX (per-scanline gradients, wave), windows
// (smooth letterbox), typewriter CJK captions, PSG micro-sfx.
//
// Mirrored into C by spec/gen-c.ts -> runtime/cine_gen.h.
...
// sheet (A-prompt 16x16 + digits 0-9 8x16) appended by the compiler.
export const OBJ_TILE_MAX = 1024;
export const PALBANK_OBJ_UI = 15;

// OAM slot plan: 0..15 scene sprites, 16..23 counter digits, 24 A-prompt.
...
export const DBG_MAGIC = 0x454e4943; // "CINE"
export const DBG = {
  MAGIC: 0x00, // u32
  BOOTED: 0x04, // u8
  SCENE: 0x05, // u8
