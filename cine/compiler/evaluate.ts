// cine/compiler/evaluate.ts — execute the declaration zone, capture cue ASTs.
...
export async function evaluateFilm(entryPath: string): Promise<EvalResult> {
  ...
  rewritten = rewritten.replace(/(["'])@pocketjs\/cine\1/g, JSON.stringify(DSL_INDEX));
  ...
  const tmp = entryPath + `.__cine.${process.pid}.${evalCounter++}.mjs`;
