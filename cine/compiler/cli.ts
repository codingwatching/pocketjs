#!/usr/bin/env bun
// cine/compiler/cli.ts — `bun cine/compiler/cli.ts build film.tsx --out out.gba`
...
if (args[0] !== "build" || !args[1]) {
  console.error("usage: cine build <film.ts> [--out dist/film.gba] [--title TITLE]");
  process.exit(1);
}
const entry = args[1];
const out = args.includes("--out") ? args[args.indexOf("--out") + 1] : new URL("../dist/film.gba", import.meta.url).pathname;
const title = args.includes("--title") ? args[args.indexOf("--title") + 1] : "CINE";
...
console.log(
  `cine: ${rom.gba} (${rom.size} bytes), ${film.scenes.length} scenes, ` +
