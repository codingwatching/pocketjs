/* cine/runtime/video.c — boot video state + whole-scene VRAM loads.
...
#define OBJ_UI_BASE 1000 /* A-prompt (4 tiles) + digits 0-9 (2 tiles each) */

Cine g;

static void bgcnt_setup(u8 wide) {
  REG_BGCNT(0) = BG_4BPP | BG_PRIO(0) | BG_CBB(C_CBB_SHARED) | BG_SBB(C_SBB_UI) | BG_SIZE_32x32;
  REG_BGCNT(1) = BG_4BPP | BG_PRIO(2) | BG_CBB(C_CBB_MAIN) | BG_SBB(C_SBB_MAIN) | BG_MOSAIC |
                 (wide ? BG_SIZE_64x32 : BG_SIZE_32x32);
...
void video_boot(void) {
  ...
  /* built-in UI OBJ sheet (A prompt + digits) at the top of OBJ VRAM */
  dma3_copy32(OBJ_VRAM + OBJ_UI_BASE * 16, film.ui_obj_tiles, film.ui_obj_bytes / 4);
  bgcnt_setup(0);
...
void scene_load(u8 id) {
  const CineScene *sc = &film.scenes[id];
  int i;
  ...
  /* maps */
  dma3_copy32(SCREENBLOCK(C_SBB_MAIN), sc->map_main, (sc->wide ? 0x1000u : 0x800u) / 4);
